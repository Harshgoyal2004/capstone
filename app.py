...
async def chat_endpoint(request: ChatRequest):
    ...
    # Inject file upload status into the latest user message so the LLM knows
    if messages and messages[-1]["role"] == "user":
        status_notes = []
        if request.ecg_file_path:
            status_notes.append("The user has already uploaded an ECG file.")
        if request.voice_file_path:
            status_notes.append("The user has already uploaded a Voice recording file.")
        
        if status_notes:
            sys_note = "\n\n[System Note to AI: " + " ".join(status_notes) + " Do not ask the user for these files again. In your <MODEL_INPUT>, mark their status as 'provided'.]"
            messages[-1]["content"] += sys_note
...
async def read_root():
    ...
    return {"status": "ok", "message": "Health Screening API is running."}
